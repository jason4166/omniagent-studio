...
一个把知识回答和人工审批操作放进同一套 Runtime 的工程项目。[在线体验](https://omniagentstudio.top) 已于 2026-09-21 通过可信 TLS 公开访问，备案号为津ICP备2026013554号-1。登录页预填“管理只读”访客入口，可直接登录；每个独立浏览器获得隔离的临时身份、会话和审批权限。运行源码 `a8b9000e0a0619bcb044b48d1b44cbb72e8d5459` 与验收范围见 [正式上线记录](public-launch-2026-09-21.md)；自部署使用 [Quickstart](../README.md#快速启动)。下方既有云端截图对应的历史版本见 [9 月 12 日部署记录](cloud-deployment-2026-09-12.md)。
...
本次路由修复补充了工具用途和返回字段契约：先判断工具能否回答实际问题，再追问其缺失参数；带产品型号的政策问题仍应检索授权知识。预置描述更新保留用户自定义配置，具体版本见 [正式上线记录](public-launch-2026-09-21.md)。
...
- **本次公网上线**：[正式上线记录](public-launch-2026-09-21.md) 包含独立 production 新卷、外网 DNS/TLS/端口检查、`public_smoke`、4 条浏览器 E2E 和一次加密备份恢复验证。每日服务器本地加密备份与每小时 TTL 清理已启用，并已将加密备份复制到工作站。
...
- **当前限制**：自动异地备份和外部告警监控尚未实现；评测使用合成数据集，价格未知时成本为 unknown，Fake 保留一条安全拒答的评测失败。见 [已知限制](failures-and-limitations.md)。
