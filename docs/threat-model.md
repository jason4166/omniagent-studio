...
```mermaid
flowchart LR
  U[Untrusted browser / user input] --> B[Bearer authentication · size and rate limits]
...
| --- | --- | --- |
| Direct injection / forged roles | DevUserContext is server-owned; user headers do not choose roles | security matrix SEC-01–05, 13–15; eval attacks |
...
| Resource exhaustion | Body/upload/context/response bounds; deadlines; retry cap; SSE stream cap; process-local rate limit | boundary and fault tests |
